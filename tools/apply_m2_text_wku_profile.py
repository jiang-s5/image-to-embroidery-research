...
def candidate_score(candidate: dict[str, Any], base: dict[str, Any], args: argparse.Namespace) -> float:
    precision_gain = metric_delta(candidate, base, "adaptive_stitch_precision_ratio")
    ...
    strict_deficit = max(0.0, args.min_strict_precision - safe_float(candidate.get("stitch_precision_ratio")))
    return round(
        ...
        + args.strict_deficit_weight * strict_deficit
        - args.adaptive_precision_gain_weight * precision_gain,
        8,
    )
...
def passes_gate(candidate: dict[str, Any], base: dict[str, Any], args: argparse.Namespace) -> tuple[bool, str]:
    ...
    if safe_float(base.get("adaptive_stitch_precision_ratio")) >= args.trigger_below_adaptive_precision:
        return False, "base_precision_not_low"
    precision_gain = metric_delta(candidate, base, "adaptive_stitch_precision_ratio")
    ...
    if precision_gain < args.min_adaptive_precision_gain:
    ...
    return True, "text_low_adaptive_precision_gain"


def main() -> int:
    parser = argparse.ArgumentParser(description="Apply a narrow text profile for WKU-like low adaptive precision samples.")
    ...
    parser.add_argument("--trigger-below-adaptive-precision", type=float, default=0.90)
    parser.add_argument("--min-adaptive-precision", type=float, default=0.90)
    parser.add_argument("--min-adaptive-precision-gain", type=float, default=0.01)
    parser.add_argument("--min-strict-precision", type=float, default=0.75)
    parser.add_argument("--min-coverage", type=float, default=0.98)
    parser.add_argument("--min-adaptive-coverage", type=float, default=0.94)
    parser.add_argument("--max-loss-increase", type=float, default=0.01)
    parser.add_argument("--max-jump-increase", type=float, default=1.0)
    parser.add_argument("--max-trim-increase", type=float, default=1.0)
    ...
    parser.add_argument("--strict-deficit-weight", type=float, default=0.08)
    ...
    for base_row in base_rows:
        ...
        current["text_wku_profile_reason"] = "non_text_source"
        ...
        for candidate in scan_rows_by_sample.get(sample_id, []):
            ...
            if not allowed:
                rejection_reasons.append(f"{candidate.get('candidate','')}:{reason}")
                continue
            candidates.append((candidate_score(candidate, base_row, args), candidate, reason))

        if not candidates:
            decision.update({"reason": ";".join(rejection_reasons[:12]) or "no_candidate"})
            copy_outputs(base_root, sample_id, output_dir)
            selected.append(current)
            decisions.append(decision)
            continue
        ...
        candidate_root = results_root / chosen["candidate"]
        ...
        decision.update(
            {
                "selected_profile": "text_wku_precision",
                "selected_candidate": chosen["candidate"],
                "decision": "switched",
                "reason": reason,
                "selector_score": score,
                "adaptive_precision_gain": metric_delta(chosen, base_row, "adaptive_stitch_precision_ratio"),
                "strict_precision_gain": metric_delta(chosen, base_row, "stitch_precision_ratio"),
                ...
                "jump_delta": metric_delta(chosen, base_row, "jump_count"),
    ...
    summary = {
        "model_id": "m2_69_text_wku_precision_profile",
        "base_profile_system": "m2_68_shape_aware_profile",
        ...
        "switched_samples": [
            {
                ...
                "adaptive_precision_gain": row.get("delta_vs_base_adaptive_stitch_precision_ratio", ""),
                "strict_precision_gain": row.get("delta_vs_base_stitch_precision_ratio", ""),
    ...
    write_csv(selected, output_dir / "text_wku_profile_selected_rows.csv")
    write_csv(decisions, output_dir / "text_wku_profile_decision_rows.csv")
    write_csv(switched, output_dir / "text_wku_profile_switched_rows.csv")
    write_csv(by_profile, output_dir / "text_wku_profile_by_profile.csv")
    write_csv(by_source, output_dir / "text_wku_profile_by_source.csv")
    write_csv(by_category, output_dir / "text_wku_profile_by_category.csv")
    write_json(summary, output_dir / "text_wku_profile_summary.json")
