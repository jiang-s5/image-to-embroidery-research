...
def load_line_candidate_rows(dataset_dir: Path, line_root: Path, line_rows_csv: Path, target_width_mm: float) -> dict[str, dict[str, Any]]:
    manifest = {row["sample_id"]: row for row in read_csv(dataset_dir / "manifest.csv")}
    base_rows = {row["sample_id"]: row for row in read_csv(line_rows_csv)}
    rows: dict[str, dict[str, Any]] = {}
    for sample_id, row in base_rows.items():
        sample = manifest.get(sample_id)
        if sample is None:
            continue
        dst_path = line_root / sample_id / "prediction.dst"
        ...
        candidate.update(adaptive)
        candidate["candidate_root"] = str(line_root)
        candidate["candidate_name"] = line_root.name
        rows[sample_id] = candidate
    return rows


def line_profile_is_worth_it(candidate: dict[str, Any], baseline: dict[str, Any], args: argparse.Namespace) -> tuple[bool, str]:
    ...
    overfill_reduction = -metric_delta(candidate, baseline, "overfill_outside_adaptive_ratio")
    ...
    if overfill_reduction < args.min_overfill_reduction:
    ...
    return True, "line_art_precision_gain"


def line_profile_score(candidate: dict[str, Any], baseline: dict[str, Any], args: argparse.Namespace) -> float:
    adaptive_precision_gain = metric_delta(candidate, baseline, "adaptive_stitch_precision_ratio")
    strict_precision_gain = metric_delta(candidate, baseline, "stitch_precision_ratio")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply a QuickDraw/line-art precision profile on top of an existing M2 profile selection.")
    parser.add_argument("--base-selection", required=True)
    parser.add_argument("--base-output-dir", required=True)
    parser.add_argument("--line-output-dir", required=True)
    parser.add_argument("--line-rows-csv", required=True)
    parser.add_argument("--line-candidate", action="append", nargs=2, metavar=("OUTPUT_DIR", "ROWS_CSV"), default=[])
    ...
    parser.add_argument("--min-adaptive-precision-gain", type=float, default=0.10)
    parser.add_argument("--min-strict-precision-gain", type=float, default=0.03)
    parser.add_argument("--min-overfill-reduction", type=float, default=0.05)
    parser.add_argument("--min-adaptive-precision", type=float, default=0.95)
    parser.add_argument("--min-adaptive-coverage", type=float, default=0.84)
    parser.add_argument("--max-loss-increase", type=float, default=0.18)
    parser.add_argument("--max-jump-increase", type=float, default=22.0)
    ...
    parser.add_argument("--target-adaptive-coverage", type=float, default=0.84)
    parser.add_argument("--target-coverage", type=float, default=0.92)
    ...
    parser.add_argument("--coverage-deficit-weight", type=float, default=0.04)
    parser.add_argument("--adaptive-precision-gain-weight", type=float, default=0.08)
    parser.add_argument("--strict-precision-gain-weight", type=float, default=0.05)
    args = parser.parse_args()

    dataset_dir = Path(args.dataset_dir)
    output_dir = Path(args.output_dir)
    base_root = Path(args.base_output_dir)
    line_root = Path(args.line_output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    base_rows = read_csv(Path(args.base_selection))
    candidate_sets: list[tuple[Path, dict[str, dict[str, Any]]]] = [
        (line_root, load_line_candidate_rows(dataset_dir, line_root, Path(args.line_rows_csv), args.target_width_mm))
    ]
    for candidate_dir, rows_csv in args.line_candidate:
        root = Path(candidate_dir)
        candidate_sets.append((root, load_line_candidate_rows(dataset_dir, root, Path(rows_csv), args.target_width_mm)))
    ...
    for base in base_rows:
        ...
        if base.get("source_name") == args.line_source_name:
            rejection_reasons: list[str] = []
            for candidate_root, candidate_rows in candidate_sets:
                candidate = candidate_rows.get(sample_id)
                if candidate is None:
                    continue
                allowed, gate_reason = line_profile_is_worth_it(candidate, base, args)
                if allowed:
                    candidate_pool.append((line_profile_score(candidate, base, args), candidate_root, candidate, gate_reason))
                else:
            ...
            if candidate_pool:
                ...
                reason = f"{reason}|line_art_rejected:{';'.join(rejection_reasons) or 'no_candidate'}"
        ...
        decisions.append(
            {
                ...
                "overfill_reduction": -metric_delta(chosen, base, "overfill_outside_adaptive_ratio"),
                ...
                "off_mask_delta": metric_delta(chosen, base, "off_mask_stitch_length_mm"),
            }
        )
    ...
    summary = {
        "model_id": "m2_67_line_art_profile",
        "samples": len(selected),
        "base_profile_system": "m2_66_profile_switcher",
        "line_art_profile": "candidate_pool",
        "line_art_candidate_roots": [root.name for root, _rows in candidate_sets],
    ...
    write_csv(selected, output_dir / "line_art_profile_selected_rows.csv")
    write_csv(decisions, output_dir / "line_art_profile_decision_rows.csv")
    write_csv(switched, output_dir / "line_art_profile_switched_rows.csv")
    write_csv(by_profile, output_dir / "line_art_profile_by_profile.csv")
    write_csv(by_source, output_dir / "line_art_profile_by_source.csv")
    write_csv(by_category, output_dir / "line_art_profile_by_category.csv")
    write_json(summary, output_dir / "line_art_profile_summary.json")
