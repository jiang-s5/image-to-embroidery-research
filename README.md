...
```text
configs/m2_89_hard_failure_aware_selector.json
results/public_benchmark_v1_ext33_m2_89_hard_failure_deployment_strict/
```

M2.89 is the current learned-control training package. It uses M2.88 no-gate false positives as high-weight hard negatives and adds explicit execution-risk features for jump, trim, off-mask, visible-connector, and hard-failure explosions. Its strict deployment profile preserves the M2.86/M2.88 final DST outputs: `0.057069` mean unified loss, `6.151515` mean jump count, `0.981679` coverage, `0.839183` stitch precision, `0.735441` professional preview score, and `0.226121` generator texture score across 33 public benchmark images, with `0` hard_fail and `0` visible connectors. The key gain is robustness: the M2.88 no-gate replay failed with `3` hard_fail samples and `59.424242` mean jumps, while the M2.89 no-gate replay returns to `0` hard_fail and `6.151515` mean jumps. M2.89 is therefore a hard-failure-aware learned selector refresh, not yet a new professional-preview quality leap.

The current strongest professional-texture experiment is:

```text
configs/m2_90_professional_texture_objective.json
results/public_benchmark_v1_ext33_m2_90_professional_texture_objective/
```

M2.90 changes the optimization target from safety-only selection to explicit DST-derived professional texture. The hard-texture profile raises mean professional preview score from `0.735441` to `0.779045` and mean generator texture score from `0.226121` to `0.699091` across the same 33 public benchmark images, while keeping `0` hard_fail and `0` visible connectors. The tradeoff is intentional: mean unified loss rises to `0.089982`, mean jumps rise to `9.878788`, and mean stitch precision drops to `0.826331`. M2.90 is therefore the current best experimental profile for professional-looking stitch texture, while M2.89 remains the conservative safety baseline. See [docs/m2_90_professional_texture_objective.md](docs/m2_90_professional_texture_objective.md).

M2.74 remains an important earlier professional-texture profile. It promotes four shape/icon samples to satin-like outputs while keeping `0` hard_fail, `0` visible connectors, no off-mask increase, and `0` low-adaptive-precision samples. Its public benchmark metrics were `0.056399` mean unified loss, `6.030303` mean jump count, `0.981039` coverage, `0.837796` strict precision, `0.967046` adaptive stitch precision, and `0.208606` mean texture score across 33 public benchmark images.
...
M2.87 converts M2.86's preview-positive and preview-rejected candidates into explicit teacher rows, then trains a preview-aware stitch-family selector. The refreshed teacher set grows to `2244` rows with `73` positive teacher rows and `1149` hard negatives. The selected learned-control variant, `rejectw16_margin010`, reaches source-held-out accuracy `0.950739`, reject recall `0.959825`, non-base positive accuracy `0.681818`, and keeps false texture promotions at `0`, slightly improving M2.81's non-base positive accuracy (`0.675676`) while preserving the zero-false-promotion safety line. M2.87 is not yet a promoted DST output profile; M2.86 remains the current output selector until the M2.87 learned selector is wired into deployment and rerun on final DST metrics. See [docs/m2_87_preview_teacher_refresh_selector.md](docs/m2_87_preview_teacher_refresh_selector.md).

M2.88 wires the M2.87 selector into the final deployment path. Under the strict policy, candidates must still pass the M2.86 command/preview gate and then be accepted by the learned selector. This exactly reproduces the M2.86 final outputs on all `33` public benchmark images: `0` hard_fail, `0` visible connectors, mean unified loss `0.057069`, mean jumps `6.151515`, mean professional preview score `0.735441`, and mean generator texture score `0.226121`, with `0` sample differences from M2.86. A no-gate probe gives small preview/texture gains but creates `3` hard_fail samples and raises mean jumps to `59.424242`, so it is not promoted. M2.88 is therefore a learned-control deployment confirmation layer, not a replacement for the deterministic safety gate. See [docs/m2_88_preview_learned_deployment_policy.md](docs/m2_88_preview_learned_deployment_policy.md).

M2.89 folds those M2.88 no-gate failures back into training. It adds `6` high-weight hard-negative rows from `ocp_001`, `ocp_009`, `omj_010`, and `qd_007`, plus execution-risk features for jump explosion, trim explosion, off-mask length, visible connectors, and hard-failure replay. The refreshed selector keeps source-held-out positive accuracy and non-base positive accuracy unchanged versus M2.87 (`0.808219` and `0.681818`) with `0` false texture promotions, while the no-gate deployment replay now returns to the same safe final outputs as M2.86/M2.88 strict: `0` hard_fail, `0` visible connectors, mean unified loss `0.057069`, and mean jumps `6.151515`. M2.89 is promoted as the current learned-control package, but the output profile remains M2.86/M2.88/M2.89 strict-equivalent. See [docs/m2_89_hard_failure_aware_selector.md](docs/m2_89_hard_failure_aware_selector.md).

M2.90 adds an explicit professional-texture objective on top of the M2.89 safety baseline. It audits the same `1584` candidate rows, allows bounded execution tradeoffs, and selects `19` professional-texture switches while preserving `0` hard_fail and `0` visible connectors. The hard-texture profile improves mean professional preview score by `+0.043604`, generator texture by `+0.472970`, family texture by `+0.338101`, and coverage by `+0.010604`; the accepted tradeoff is higher unified loss (`+0.032913`), higher jumps (`+3.727273`), and lower precision (`-0.012852`). M2.90 is the current professional-texture experiment, not yet a complete commercial digitizer. See [docs/m2_90_professional_texture_objective.md](docs/m2_90_professional_texture_objective.md).
