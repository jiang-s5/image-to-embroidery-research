...
```text
configs/m2_88_preview_learned_deployment_policy.json
results/public_benchmark_v1_ext33_m2_88_preview_learned_deployment_policy/
```

M2.88 is the current learned-control deployment layer. Its strict profile exactly preserves the M2.86 final DST outputs while wiring the M2.87 preview-aware learned selector into deployment as a non-reject confirmation layer. The current public benchmark metrics are `0.057069` mean unified loss, `6.151515` mean jump count, `0.981679` coverage, `0.839183` stitch precision, `0.735441` professional preview score, and `0.226121` generator texture score across 33 public benchmark images, with `0` hard_fail and `0` visible connectors. The no-gate learned probe is explicitly rejected because it creates hard failures and jump explosions.
